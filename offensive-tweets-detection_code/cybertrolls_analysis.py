...
def classify_and_evaluate(file, train_data, cross_validation_data, train_label, cross_validation_label, number_features):
	
	print("Logisticregression")
	classification_start = time.time()
	lr_classifier = LogisticRegression(solver="liblinear").fit(train_data, train_label)
	classification_end = time.time()
	predictions = lr_classifier.predict(cross_validation_data)
	write_to_file(file, ["Logisticregression"] + [str(number_features), str(classification_end-classification_start)] + classification_results(cross_validation_label, predictions))

	print("SGDClassifier")
	classification_start = time.time()
	sgd_classifier = SGDClassifier().fit(train_data, train_label)
	classification_end = time.time()
	predictions = sgd_classifier.predict(cross_validation_data)
	write_to_file(file, ["SGDClassifier"] + [str(number_features), str(classification_end-classification_start)] + classification_results(cross_validation_label, predictions))

	print("LinearSVC")
	classification_start = time.time()
	lin_svc = LinearSVC().fit(train_data, train_label)
	classification_end = time.time()
	predictions = lin_svc.predict(cross_validation_data)
	write_to_file(file, ["LinearSVC"] + [str(number_features), str(classification_end-classification_start)] + classification_results(cross_validation_label, predictions))
	'''
	print("NuSVCClassifier")
	classification_start = time.time()
	nu_svc_classifier = NuSVC(gamma='auto').fit(train_data, train_label)
	classification_end = time.time()
	predictions = nu_svc_classifier.predict(cross_validation_data)
	write_to_file(file, ["NuSVCClassifier"] + [str(number_features), str(classification_end-classification_start)] + classification_results(cross_validation_label, predictions))

	print("SVCClassifier")
	classification_start = time.time()
	svc_classifier = SVC(gamma='auto').fit(train_data, train_label)
	classification_end = time.time()
	predictions = svc_classifier.predict(cross_validation_data)
	write_to_file(file, ["SVCClassifier"] + [str(number_features), str(classification_end-classification_start)] + classification_results(cross_validation_label, predictions))
	'''
	print("GaussianNB")
	classification_start = time.time()
	gaussian_classifier = GaussianNB().fit(train_data, train_label)
	classification_end = time.time()
	predictions = gaussian_classifier.predict(cross_validation_data)
	write_to_file(file, ["GaussianNB"] + [str(number_features), str(classification_end-classification_start)]+ classification_results(cross_validation_label, predictions))

	print("DecisionTreeClassifier")
	classification_start = time.time()
	tree_classifier = DecisionTreeClassifier().fit(train_data, train_label)
	classification_end = time.time()
	predictions = tree_classifier.predict(cross_validation_data)
	write_to_file(file, ["DecisionTreeClassifier"] + [str(number_features), str(classification_end-classification_start)] + classification_results(cross_validation_label, predictions))

	print("RandomForestClassifier")
	classification_start = time.time()
	rf_classifier = RandomForestClassifier(n_estimators=10).fit(train_data, train_label)
	classification_end = time.time()
	predictions = rf_classifier.predict(cross_validation_data)
	write_to_file(file, ["RandomForestClassifier"] + [str(number_features), str(classification_end-classification_start)] + classification_results(cross_validation_label, predictions))
	'''
	print("AdaBoostClassifier")
	classification_start = time.time()
	adaboost_classifier = AdaBoostClassifier().fit(train_data, train_label)
	classification_end = time.time()
	predictions = adaboost_classifier.predict(cross_validation_data)
	write_to_file(file, ["AdaBoostClassifier"] + [str(number_features), str(classification_end-classification_start)] + classification_results(cross_validation_label, predictions))

	print("XGBClassifier")
	classification_start = time.time()
	xg_classifier = XGBClassifier().fit(train_data, train_label)
	classification_end = time.time()
	predictions = xg_classifier.predict(cross_validation_data)
	write_to_file(file, ["XGBClassifier"] + [str(number_features), str(classification_end-classification_start)] + classification_results(cross_validation_label, predictions))
	'''
	print("MLPClassifier")
	classification_start = time.time()
	neural_net_classifier = MLPClassifier().fit(train_data, train_label)
	classification_end = time.time()
...
def perform_analysis(
	...
	for train_index, cross_validation_index in kf.split(data['content'].values):
		...
		classify_and_evaluate(file, train_data, cross_validation_data, train_label, cross_validation_label, number_features)
