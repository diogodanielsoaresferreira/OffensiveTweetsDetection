...
def classify_and_evaluate(file, train_data, cross_validation_data, train_label, cross_validation_label, number_features, parameters):
	...
	decision_tree_classifier = DecisionTreeClassifier(criterion=parameters["criterion"]).fit(train_data, train_label)
	classification_end = time.time()
	predictions = decision_tree_classifier.predict(cross_validation_data)
	write_to_file(file, ["DecisionTree"] + [str(number_features), str(classification_end - classification_start)] + classification_results(cross_validation_label, predictions))
