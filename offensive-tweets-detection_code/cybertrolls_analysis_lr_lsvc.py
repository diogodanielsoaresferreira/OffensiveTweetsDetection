...
def classify_and_evaluate(file, train_data, cross_validation_data, train_label, cross_validation_label, number_features, classifier, parameters):

	if classifier == 0:
		print("Logisticregression")
		classification_start = time.time()
		lr_classifier = LogisticRegression(penalty=parameters['penalty'], C=parameters['C'], solver=parameters['solver']).fit(train_data, train_label)
		...
		if(parameters['penalty'] == 'l1'):
			...
			dual = True
		lin_svc = LinearSVC(penalty=parameters['penalty'], C=parameters['C'], max_iter=parameters['max_iter'], dual=dual).fit(train_data, train_label)
		classification_end = time.time()
...
def perform_analysis(
	...
	for parameters_dict in parameters:
		...
		for train_index, cross_validation_index in kf.split(data['content'].values):
			...
			classify_and_evaluate(file, train_data, cross_validation_data, train_label, cross_validation_label, number_features, classifier, parameters_dict)
